...
- **Hour atomicity (PR #508 review):** each forecast hour is HRRR-or-untouched — the sounding must decode for EVERY route point (guards the pinned gate constants drifting from the real grid) and merge before that hour's diagnostics are applied, so a sounding failure with a healthy diag blob can never leave HRRR's instantaneous diagnostics under a plain-GFS fallback. Success accounting increments at sounding-merge time and the loop is exception-contained, so once anything merged the slot stays HRRR.
...
- Disk cache at `data/.cache/grib/hrrr/{date}_{cycle}z/` (6h TTL — hourly cycles supersede fast; 8 GiB size-cap backstop enforced from the daily retention pass, since HRRR is not precached and so has no warm-loop enforcement site).
