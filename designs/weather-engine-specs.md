...
- **Measured warming sizing (#475, real prod data 2026-07-22 — supersedes the earlier "a few GB/day" estimate, which was ~20× low because dev has 2 future flights and prod has 73):** DWD serves whole-domain files with no byte-range/`.idx` subsetting, so warming downloads all of Germany to sample a 10 NM corridor — the only levers are fewer hours/levels/runs.
  ...
  - **Per forecast-hour** (full column; #469 phase 2 gated off, see #474): 9 vars × 50 levels (16–65) = 450 files ≈ 427 MB download / 730 MB disk.
  - **Prod load:** 73 future flights, 20–26 D2-eligible per run, per-run union 21–29 forecast-hours. All 8 passes/day = 817 forecast-hours ≈ **71.8 GB/day**; the 03Z–21Z window = 618 forecast-hours ≈ **54.3 GB/day** (24% saving, ~83 GB less over the measured ~4.75-day window).
...
- **Cache retention (#475):** three changes land the warming safely on prod's ~52 GB free.
  ...
  - **Size-cap backstop** (`cache_cap_bytes` → `_enforce_size_cap`): after the TTL rule, evict whole run dirs oldest-init-first until the model total fits `WB_GRIB_CACHE_CAP_GB_<MODEL>` (default 45 GiB for `icon-d2`), never below a floor of 2 runs (current + prior fallback). Whole-dir, not per-file: per-level re-download would trade scarce DWD bandwidth for disk. Not LRU — a stale run is stale regardless of read recency, so init age is the right signal. Every eviction is logged.
...
- **Ceiling-limited fetch (#469 phase 2) — IMPLEMENTED BUT GATED OFF (`WB_ICON_CEILING_LIMIT_ENABLED`, default false):** held back because the *fetch* cut is safe but the sounding *consumers* are not yet safe for a truncated column, and each failure reads as REASSURING rather than unavailable. (a) The cut is asymmetric by necessity (t/qv/p full column for CAPE), so pressure levels above it carry temperature but **no wind**; `analysis/sounding/prepare.py` gates wind all-or-nothing (`has_wind = all(...)`), so one wind-less level drops wind for the **whole** profile → no Richardson number → `cat_risk_layers == []` → the turbulence advisory grades GREEN "smooth" instead of UNAVAILABLE (its guard keys on the vertical-motion assessment, which omega keeps alive) — the #391/#393 "absence reads as clear" failure exactly. (b) Truncating `clc` caps NWP cloud decks at the cut, so a deck above it disappears and reads as "model clear" to the cloud-top advisory, the DD/NWP agreement check and the digest. (c) The cross-section renders to ceiling + 5,000 ft and the Skew-T to 250 hPa, so both show an unexplained blank top strip for ICON only. Re-landing needs those consumers to report a truncated column honestly (per-level wind gate mirroring the omega `any()`+NaN pattern, an "above the fetched cut" sentinel distinct from "clear", clipped/labelled charts) — or a **uniform** cut across all variables, which removes the mixed-column problem but must then solve CAPE. Phases 1 and 3 are independent of this flag and stay active. The machinery below stays in the tree and unit-tested. Design as implemented: the wind/cloud sounding variables (`u, v, w, qc, qi, clc`) carry nothing useful above the flight ceiling, so they are fetched only down to a domain-safe model-level cut derived from `flight_ceiling_ft` (`icon_levels_by_var` → `_IconEuContext.levels_by_var`; ~19% of a D2 run's volume when cut at FL180). The cut uses `ICON_D2_CEILING_LEVEL_CUTS`, anchors of (sea-level HHL height → top level) — level 27 = 18,680 ft, 30 = 16,096 ft, 38 = 10,354 ft. Model levels ride HIGHER over terrain (level 27 is 20,549 ft over the Alps), so the SEA-LEVEL column is the binding case: a cut chosen from sea-level heights covers the ceiling everywhere in the domain. **Hard constraint:** `t, qv, p` (`FULL_COLUMN_VARIABLES`) stay at the full column regardless of ceiling — MetPy CAPE integrates to an equilibrium level routinely far above the flight (live ESMX→EKRK: EL FL306, cruise 6,000 ft; echo tops FL365), so truncating them would under-compute CAPE and cap tower tops. A ceiling above the deepest anchor (or None, or ICON-EU) → full column (fail-safe: no truncation). Above the cut the log-pressure interpolation simply drops those targets for the limited variables (no extrapolation). Composes with phase 1: a later briefing with a lower ceiling tops up only the extra levels it needs.
- **Per-level cache layout (#469 phase 1, `per_level_cache=True`):** the model-level sounding is cached one file per (variable, level) — `f029_ICON_D2_T_L27.grib2` (`icon_model_level_var_label`) — not one whole-column blob per variable like ICON-EU. This is a *correctness* prerequisite for the ceiling-limited fetch, not just an efficiency: a whole-column cache key can't distinguish a complete column from a partial one, so writing a subset of levels and then reporting the key cached would silently serve a truncated sounding. Per level, `is_cached` answers per level, so a briefing needing deeper levels tops up only the missing files. The prefetch (`fetch_icon_eu_per_level`) and decode (`decode_icon_chunked`, which now concatenates a per-variable *list* of level files) share the path via the `per_level_cache` flag; cfgrib indexes by the level coordinate so concatenation order is irrelevant. Migration is free: whole-column blobs written by older code — both `{prefix}_{VAR}` (`icon_model_level_var_legacy_label`) and the even-older `{prefix}_QC_QI_P` — are treated as a hit for the full range and preferred when present, so no cache flush is needed. ICON-EU keeps the whole-column blob (its 40-level column is cheap and no EU path fetches a subset).
