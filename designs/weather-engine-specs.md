...
- **Flight-driven cache warming (#469 phase 3):** after each D2 run publishes, `precache_icon_d2_flights` warms the D2 cache for the *actual* flights in the DB departing within the next 48h (D2's horizon), using their real routes, window hours, and ceilings. It reuses the briefing prepare+prefetch path verbatim (`_prepare_icon_eu` → `_prefetch_icon_eu_data`), so the warmed cache is byte-for-byte what an on-demand briefing reads — same run, same per-level files (and the same ceiling limiting, whenever phase 2's gate is on) — giving a near-100% hit rate at a few GB/day. This deliberately **rejects** a broad daylight-band precache (sized at 37–75 GB/day, ~20% hit rate, 2–4× more DWD bandwidth than on-demand). Only D2-eligible flights are warmed here (EU warming is the airport-profile precache's job). Wired into `run_grib_precache_loop` on every fresh `icon_d2:dwd` marker (all 8 cycles, keyed `icon_d2:flights` so it can't collide with the EU/GFS `last_done` entries); gated by `WB_GRIB_PRECACHE_ENABLED` (off in dev).
- **Ceiling-limited fetch (#469 phase 2) — RE-LANDED (#474), DEFAULT ON (`WB_ICON_CEILING_LIMIT_ENABLED`, default true; set `false` to re-measure the marginal saving now that phase-3 warming runs at a near-100% hit rate):** the *fetch* cut is asymmetric by necessity (t/qv/p full column for CAPE), so pressure levels above it carry temperature but **no wind and no cloud** — and the consumers now report that truncated strip honestly instead of as REASSURING (approach A: fix consumers, keep the cut). (a) `analysis/sounding/prepare.py` gates wind **per level** (NaN fill, mirroring the omega `any()` branch), so the flyable column below the cut keeps real wind → Richardson / CAT still grade at cruise and turbulence no longer reads a false GREEN "smooth" (the old #391/#393 failure); `_compute_bulk_shear` returns None (not NaN) when its top level is truncated. (b) `build_nwp_cloud_layers_from_fraction` flags a deck `top_truncated` when the cut bounds its top, `SoundingAnalysis.fetched_column_top_ft` records the fetched top (highest wind-carrying level), and the cloud-top / DD-vs-NWP consumers treat the strip above it as unknown (never toppable, never "model clear"). The DD cloud track is built from the always-full t/qv column and is unaffected. (c) The cross-section and Skew-T draw a labelled "not modeled above FLxxx" reference line at `fetched_column_top_ft` (web + iOS) so the ICON-only blank strip above the cut reads as absent-data, not calm/clear. Phases 1 and 3 are independent of this flag and stay active. Design as implemented: the wind/cloud sounding variables (`u, v, w, qc, qi, clc`) carry nothing useful above what's displayed, so they are fetched only down to a domain-safe model-level cut derived from `flight_ceiling_ft` (`icon_levels_by_var` → `_IconEuContext.levels_by_var`). **The cut covers `flight_ceiling_ft + CROSS_SECTION_DISPLAY_BUFFER_FT` (5,000 ft), NOT the bare ceiling** — the cross-section / Skew-T render wind/cloud to `ceiling + 5,000` (the clients hard-code the same `CROSS_SECTION_DISPLAY_BUFFER_FT` in `web/ts/visualization/types.ts` and iOS `VizRouteData.displayBufferFt`), so cutting at the bare ceiling would leave ICON-D2 blank across that whole rendered band while the full-column models fill it (#474). Consequence: only lower-ceiling flights truncate — with the 5,000 ft buffer a typical FL180 flight (18,000 + 5,000 = 23,000 ft) falls above the deepest anchor → full column; an 8,000 ft ceiling (→ 13,000 ft target) cuts at level 30. The cut uses `ICON_D2_CEILING_LEVEL_CUTS`, anchors of (sea-level HHL height → top level) — level 27 = 18,680 ft, 30 = 16,096 ft, 38 = 10,354 ft. Model levels ride HIGHER over terrain (level 27 is 20,549 ft over the Alps), so the SEA-LEVEL column is the binding case: a cut chosen from sea-level heights covers the target everywhere in the domain. **Hard constraint:** `t, qv, p` (`FULL_COLUMN_VARIABLES`) stay at the full column regardless of ceiling — MetPy CAPE integrates to an equilibrium level routinely far above the flight (live ESMX→EKRK: EL FL306, cruise 6,000 ft; echo tops FL365), so truncating them would under-compute CAPE and cap tower tops. A coverage target above the deepest anchor (or None ceiling, or ICON-EU) → full column (fail-safe: no truncation). Above the cut the log-pressure interpolation simply drops those targets for the limited variables (no extrapolation). Composes with phase 1: a later briefing with a lower ceiling tops up only the extra levels it needs.
