...
GRIB2 enrichment for Cloud Liquid Water, Ice Mixing Ratio, and cloud diagnostics — variables not available from Open-Meteo. Supports GFS (via NOAA S3), HRRR (via NOAA S3 — upgrades the gfs slot on CONUS routes, #457), ICON-EU/ICON-D2 (via DWD opendata), and ECMWF IFS (via ECPDS local delivery). Enabled via `BriefingOptions.enrich_grib=True` (always on in API, opt-in via `--enrich-grib` in CLI).
...
| Module | Purpose |
|--------|---------|
| `gfs_idx.py` | Parse `.idx` files, plan HTTP byte ranges for CLMR/ICMR and cloud diagnostic variables. Parsers/planners parametrized on variable sets — also emit the HRRR sounding/diag sets (`plan_hrrr_sounding_byte_ranges` / `plan_hrrr_diag_byte_ranges`, #457) with an "mb" level wildcard; GFS defaults unchanged |
| `grib_fetch.py` | Find latest GFS run, bracket forecast hours, download via HTTP Range from S3. `fetch_idx` / `fetch_byte_ranges` / `fetch_cloud_diag_ranges` take a keyword-only `url_builder=` (GFS default) so HRRR reuses the same parallel range-download machinery (#457) |
| `hrrr_fetch.py` | HRRR S3 URLs (flat `conus/` layout, 2-digit fhour), run selection (hourly cycles — 00/06/12/18z → 48h, else 18h; ~1h publish delay; probes the last-needed fhour's idx), Lambert domain gate (pyproj, exact x/y bounds, all-or-nothing), window hours (#457) |
...
| `precache.py` | Pre-warms ICON-EU/GFS byte-range cache for the `/maps.html` D-0..D-3 airport-profile selectables (issue #126) plus the ICON-D2 cache for upcoming flights (#469). Shares the per-flight disk cache so warmed runs also speed briefings. Warming is discretionary: it runs only inside the per-model wall-clock window (`MODEL_WARMING_WINDOW_UTC`, #475) and **yields to interactive briefings** — `interactive_refresh_active()` polls `refresh_registry.idle_seconds()` only where a real download is pending — before each EU variable/cloud-diag fetch, before each uncached GFS hour, and — via the prefetch's `abort_if` — between every D2 `(fhour, variable)` download unit, matching the EU/GFS granularity. It was once-per-D2-flight until #501, which left a ~80 s window a warm pass could not step out of. The check never runs ahead of a free `is_cached` hit, so a resumption pass fast-forwards through the warmed prefix even mid-burst; a flight stopped part-way is left partly warmed (finished units are cached and skipped next tick) and is not counted in `flights_warmed`. A gated pass returns `deferred=1` so the scheduler leaves `last_done` unset and the next 5-min tick resumes (#490; gate placement per PR #498 review, granularity per #501). Registry entries older than 30 min (`STALE_ENTRY_SECONDS`) count as leaked, not active, so a missed unregister can't disable warming until restart |
...
**HRRR enrichment — CONUS upgrade of the gfs slot (#457, `_enrich_hrrr_replace()`):**
0. **Gate (all-or-nothing, top of `_enrich_gfs_inner`):** when `WB_HRRR_ENABLED` ≠ `"0"` (kill switch) AND `route_in_hrrr_domain(route_points)` (every point projects inside the Lambert grid — exact fractional-index bounds via pyproj, not a lat/lon bbox, because the grid's edges don't follow lat/lon lines) AND `find_latest_hrrr_run(departure, cover_until=window_end)` finds a run, the gfs slot is sourced from HRRR as a **full sounding replacement**; otherwise the plain-GFS patch path above runs unchanged. Never a per-point mix.
1. **Run selection:** cycles run hourly — 00/06/12/18z reach 48h, all others 18h; ~1h publish delay with progressive publication, so the freshest eligible cycle is accepted only when the **last-needed** fhour's `.idx` answers HEAD 200 (probing f000 would accept runs whose later hours aren't published yet).
2. **Per fhour** (`hrrr_window_hours`, 1-hourly snap clamped to f48): fetch the shared `.idx` → `plan_hrrr_sounding_byte_ranges` (TMP/DPT/RH/UGRD/VGRD/VVEL/HGT/CLMR/CIMIXR × 40 pressure levels + PRES:surface; ≈190 MB/fhour) → byte-range download via `fetch_cloud_diag_ranges(..., url_builder=hrrr_grib2_url)` → disk cache `model="hrrr"`, `cache_key(fhour, "HRRR_SND")`, `data/.cache/grib/hrrr/{date}_{cycle}z/`, 6h TTL → pool decode (`decode_hrrr_pressure`; Lambert branch — pyproj-projected targets, vectorised bilinear on the y/x axes, grid→earth wind rotation) → `_replace_pressure_levels_from_grib(..., model_source=ModelSource.GFS)` — the ECMWF-shaped full-column rebuild; DPT/HGT/VVEL consumed directly (no Magnus / hypsometric / −ρg·w derivations).
3. **Guard:** zero replaced hours (or no idx at all) → return None → the gate falls through to plain GFS; an unexpected crash on the replacement path is caught at the gate and likewise falls back — never a half-HRRR pack, never a failed briefing. Per-fhour failures only leave that hour on Open-Meteo levels; the fill pass interpolates gap hours between the bracketing 40-level HRRR anchors (level-count heuristic).
4. **Diag pass (only after the replacement landed):** `plan_hrrr_diag_byte_ranges` (~15–20 MB/fhour: LCDC/MCDC/HCDC/TCDC, HGT cloud ceiling/base, CAPE/CIN at surface + 180-0 mb, VIS, GUST) → `decode_hrrr_diag` → `build_hrrr_cloud_diagnostics` (ECMWF-shaped `NWPCloudDiagnostics`; no convective-scheme fields; CIN delivered already-negative → passthrough) attached to matching hourlies, plus `build_hrrr_surface_extras` gap-filling `visibility_m` / `wind_gusts_10m_kt` / `cape_jkg` / `convective_inhibition_jkg` only where the current value is None (Open-Meteo's gfs_seamless owns the slot's surface fields — extras never clobber). A diag-pass crash is contained: the sounding stands, diag fields stay None, attribution stays `hrrr:noaa`.
5. **Attribution:** `_enrich_gfs` returns `(ts, source_key)`; `grib_sources["gfs"]` records `hrrr:noaa` vs `gfs:noaa` → pack `model_sources["gfs"]` → the freshness bar badges `GFS (HRRR)` (`MODEL_SOURCE_BADGES` + `modelSlotLabel` in web/ts/utils.ts — the `ICON (D2)` mechanics). `gfs_init_dt` is set ONLY for `gfs:noaa`: every HRRR field is instantaneous, so the GFS averaged-window machinery (window-midpoint interp, RH/condensate gate) never runs on HRRR hours — diag hours forward-fill, soundings linearly interpolate.
...
### Key Choices
- **GFS + HRRR + ICON-EU + ECMWF IFS** — GFS for global coverage, HRRR for 3 km CONUS coverage of the gfs slot (#457), ICON-EU for higher-resolution European data (~6.5km vs ~27km), ECMWF IFS for model-native cloud microphysics on ECMWF cross-sections
- **HRRR upgrades the gfs slot all-or-nothing (#457)** — gate on kill switch (`WB_HRRR_ENABLED`) + exact Lambert domain + a run covering the whole flight window; total failure or an unexpected crash falls back to plain GFS whole, never a half-HRRR pack. Badged `GFS (HRRR)` because WRF-ARW ≠ FV3-GFS — the pack must say which model sourced the slot
...
- **HRRR flat layout, 2-digit fhour** — `hrrr.{date}/conus/hrrr.t{HH}z.wrfprsf{FF}.grib2`, no `/atmos/`; publication is progressive (~1h) — probe the last-needed fhour's `.idx`, not f000
- **HRRR winds are grid-relative** (`uvRelativeToGrid=1`) — rotated to earth-relative at decode (α = (λ−LoV)·sin 38.5°; the grid→earth sign is verified against pyproj finite differences)
- **HRRR CIMIXR decodes as cfgrib var `unknown`** (NCEP-local 0/1/82) — aliased to ice mixing ratio; safe only because the sounding blob is byte-range-planned
- **HRRR CIN is delivered already-negative** (measured 2026-07-31: 0 positive of 1.9M cells) — passthrough, never `_normalize_model_cin`
