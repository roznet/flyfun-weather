...
| Evaluator | Category | Logic | Key Parameters |
|-----------|----------|-------|----------------|
| `ConvectiveEvaluator` | convective | Route points with convective risk ≥ threshold. Altitude-aware: ignores convection whose known physical tops are below `cruise_ft - top_clearance_ft`; ICON-D2 explicit cells have unresolved geometry, because their 18-dBZ echo top is not a physical/anvil top, so they can never be cleared by this filter. **NWP-native grade (#442, meteorology-decisions §18):** colour comes from the model's own NWP tier (`convective_method` defaults to `"nwp"`), **not** `max(NWP, DD)`. For ICON-EU/ECMWF/GFS this tier comes from parameterized-scheme evidence; for ICON-D2 it comes from the separate explicit-convection decision table (§19), with `source="icon_d2"` and `method="nwp_explicit"`. Incomplete D2 detection is unavailable and falls back to the independently labelled thermo track, never scheme-quiet. **DD-trigger AMBER (`reason_code="dd_trigger"`):** a green NWP whose DD-vs-NWP cross-check reads `dd_not_corroborated` is raised to AMBER only. **MODERATE+ amber floor:** any MODERATE+ point reaching cruise forces ≥ AMBER. The per-model details-only cross-check remains anchored on the grade-driving point and never regrades. **Aggregation is unchanged:** a proposed lone-D2 AMBER floor remains an open #442-framework decision, not part of #462. | `min_risk`, `affected_pct_amber`, `affected_pct_red`, `top_clearance_ft` (2000) |
| `ConvectiveCharacterEvaluator` | convective | Second convective axis (#294), **orthogonal to severity** — grades whether route convection is *circumnavigable VFR*, never touches the `convective` grade. Per-model (realized-coverage + K/TT signals are per model; disagreement on avoidability is itself signal). Explicit D2 MODERATE/HIGH assessments count as realized even without parameterized rain/cover/tower geometry. Classifies via `classify_convective_character`: NONE/UNKNOWN → GREEN; ISOLATED/SCATTERED → AMBER; WIDESPREAD/ORGANIZED/EMBEDDED → RED. Only considers points at/above `min_risk`; `_point_embedded` relabels cells under a BKN/OVC deck, `_front_present` promotes a widespread band to ORGANIZED. Altitude-aware where a real base is available; D2 explicit geometry remains unresolved. | `min_risk` (3), `showers_mm` (0.1), `isolated_max_pct` (15), `scattered_max_pct` (40), `organized_shear_kt` (35), `base_clearance_ft` (2000) |
