...
pack_id                 INT NULL            -- FK to existing forecast pack
...
source                  ENUM('manual','inflight','postflight','g1000') DEFAULT 'manual'
...
flight_id               INT                 -- FK to existing flights table
...
def degrees_per_km(lat: float) -> float:
    ...
    return 1.0 / (111.32 * abs(math.cos(math.radians(lat))))
...
def find_matching_route_watches(pirep_lat, pirep_lon, active_watches):
    ...
    for watch in active_watches:
        ...
        # Buffer in degrees, corrected for latitude
...
async def upsert_device_token(user_id, token, environment):
    await db.execute("""
...
PIREP submitted
  ...
  → for each match:
      ...
      send via APNs to correct endpoint (sandbox/production)
...
1. **Alembic migration** — 4 new tables, no existing schema changes
