...
**Add / edit flight form** (`AddFlightView` + `AddFlightViewModel`): route entry with autocomplete (`RouteAutocompleteController`) and autorouter interpretation, departure time / duration (`DepartureTimeModel`, over-long durations clamped on load, not on the pilot's next edit), and the **aircraft picker** (with an inline `AircraftFormSheet` to create one) plus the profile picker — those live here, *not* as a flight-list dropdown. A structural edit (route/date) can't be applied in place, so the form asks Move (rewrite this flight) vs Duplicate as new flight — the iOS shape of the web's swap-Save toolbar. Edits that force a re-brief go through a cost confirm; aircraft-only edits save silently. **Import from Autorouter** links the account in place when it isn't linked (#625): the picker's `409 autorouter_not_linked` becomes a Connect state; `AutorouterLinker` fetches a signed `/autorouter/link-ticket` URL, runs it in `ASWebAuthenticationSession`, and the server returns to `flyfunweather://autorouter/callback?status=linked|error&reason=…`, after which the routes load. Settings → Services has the same Connect / Disconnect.
...
- **Cross-Section** — Canvas renderer aligned with the web (smooth/soft cloud bands). Layer stack under `Views/CrossSection/Layers/`: cloud bands (soft + natural), icing (model, Ogimet NWP, SFIP), CAT, inversions, convective towers/background (thermo + NWP), terrain, temperature and stability lines, reference lines, plus a `HighlightLayer` that draws an advisory's highlight geometry (scrim + verdict ribbon, #374). Layer control is the web's family bar (#605): seven `LayerFamily` chips — compact on/off chips under the chart on iPhone portrait, one scrolling strip in iPhone landscape, the full `Family · value` bar with a swapping detail row above the chart on iPad — plus two composing lens selectors, Focus (advisory lenses) × Emulate (GRAMET / Windy / ForeFlight / FlyFun), resolved through the briefing's graded methods (`CrossSectionPresets`). Tap or sideways-drag to scrub; a vertical drag scrolls the page (`ScrubPanGesture`). See ios-app-ui.md → Cross-section layer bar. Below the chart: `RouteGraphView` (per-metric route profile) and an embedded `SkewTTabView` → `SkewTDetailView`, which renders `SkewTView` from RZSkewT with thermodynamics, wind barbs, CAPE/CIN shading, LCL/LFC/EL markers, FL labels.
...
**Help & feedback**: `HelpCatalogStore` mirrors the web help catalog to disk (raw bytes + `ETag` in `UserDefaults`, bundled `metrics-catalog.json` baseline for first run) so `HelpInfoButton` (i) popups render offline. The web is the single source of truth — no help text is hand-written in Swift. `FeedbackFormView` posts free-text feedback — app-level from the flight list, or pinned to the flight + pack on screen from a briefing's pack menu ("Send Feedback on This Briefing", #616), so admin can open the briefing the report is about; Settings deep-links to the web *settings* page (not the site root) for preferences/aircraft/profiles/services.
