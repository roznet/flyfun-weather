...
### fetch
Weather data retrieval: Open-Meteo multi-point client, route interpolation, route-aware text forecasts (NWS AFD US / DWD Europe), SRTM elevation, model freshness, and GRIB2 enrichment (GFS + HRRR-on-CONUS + ICON-EU/D2 + ECMWF IFS) with two-phase sequential decode for memory safety.
...
GRIB2 enrichment engine: GFS S3 (CLWMR/ICMR/cloud diagnostics), HRRR S3 (CONUS 3 km full sounding replacement for the gfs slot, #457), ICON-EU DWD (QC/QI/cloud cover/ceiling), ECMWF IFS ECPDS (clwc/ciwc/cc/surface diagnostics), data source registry with bucket paths, variable reference tables, implementation gotchas, future extensions.
