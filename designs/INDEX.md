...
### flight-trips
Grouping flights into a **trip** — a conjunctive feasibility chain whose viability is the AND of its *remaining* legs, with a shrinking scope as legs are flown. Pure `TripSummary` (binding leg, chain status, decision ripeness) computed per read and never persisted; two aggregations kept strictly apart so a beyond-horizon `outlook` is never folded into a traffic light. Serial one-leg-at-a-time server-driven refresh (a fan-out fails outright against `MAX_PER_USER=2` and a 2-worker executor — `"trip"` must never join `UNCAPPED_TRIGGERS`); trip auto-refresh via `_find_due_flights` trip-mates; one coalesced notification per chain; Haiku trip paragraph gated by inherited `llm_digest_enabled` with a binding-leg guardrail. Move inherits membership, Duplicate does not. The trip view never prints a verdict for the trip. Sharing reuses the per-leg `flights.private` switch rather than adding a trip-level one and is **all-or-nothing** — a trip opens for a non-owner only when every leg is public, because a chain summarised over a visible subset would name the wrong binding leg; reads are viewer-aware, every mutation (refresh above all, billed to the owner and admitted against their queue) stays owner-only, `/t/{code}` mirrors `/s/{code}`, and following a shared trip is a loop over `subscribe_flight` with no trip-subscription row.
Key exports: `summarize_trip`, `TripSummary`, `TripLegInput`, `FlightTripRow`, `trip_members`, `prune_empty_trips`, `trip_refresh.start`, `open_scheduler_run`, `ensure_trip_ai_summary`, `check_guardrail`, `bulk_trip_refs`, `shareable_trip_ids`, `load_trip_row_for_viewer`, `shared_trip_refs`
→ Full doc: flight-trips.md
→ iOS port plan: plans/ios-trips.md
...
Key exports: `compute_cost`, `compute_call_cost`, `compute_program_cost`, `CostBreakdown`, `CostConfig`, `charge_briefing`, `get_active_cost_config`, `build_program_report`, `economics_from_report`, `personal_impact`, `usage_footprint`, `donate_nudge.decide`
