...
| `NWPExplicitConvectiveDiagnostics` | Explicit-convection storm fields (ICON-D2, #462) | source ("icon_d2"), reflectivity_hour_max_dbz, reflectivity_instant_dbz (unfetched v1), echo_top_18dbz_ft (NEVER a cloud top), lightning_potential_hour_max_jkg, updraft_hour_max_ms, updraft_helicity_2_8km_hour_max_m2s2 (signed), graupel_hour_mm, detection/strength/echo_top_complete. All values are **corridor extrema** over a ~10 NM route buffer; interval-max fields describe `(H−1, H]`; None ≠ 0 — the completeness flags say whether None means quiet or unavailable |
