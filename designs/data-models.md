...
| `NWPExplicitConvectiveDiagnostics` | Convection-permitting evidence, separate from parameterized scheme output | ICON-D2 10 NM corridor maxima for hourly reflectivity, LPI, updraft, signed UH and de-accumulated graupel; constructed 18-dBZ echo top; source plus detection/strength/echo completeness. Echo top is character detail, never cloud-top geometry. |
...
| `ConvectiveAssessment` | Convective risk from indices or native model evidence | risk_level (NONE→EXTREME), CAPE/CIN, LCL/LFC/EL, bulk shear, k_index, total_totals, severe_modifiers, regime (`ConvectiveRegime`), drivers/suppressors lists, elevated_convection, base_ft/top_ft (unified bounds), cover_pct (NWP only), convective_precip_mm_h (parameterized firing signal, #283), method (including `nwp_explicit`), explicit source/completeness and character fields. `nwp_explicit` deliberately leaves base/top unset. |
