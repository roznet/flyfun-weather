...
- `verification.py` — forecast-vs-observation verification records **plus** the daily verification-digest / admin-digest payload models (`VerificationDigestData`, `AdminDigestData` + their section models)
...
Per-level NWP data. Core fields: `pressure_hpa`, `temperature_c`, `dewpoint_c`, `relative_humidity_pct`, `wind_speed_kt`, `wind_direction_deg`, `geopotential_height_m`, `vertical_velocity_pa_s` (omega). Optional GRIB2-enriched fields: `cloud_liquid_water_kg_kg` (CLWMR), `ice_mixing_ratio_kg_kg` (ICMR), `rain_water_kg_kg` / `snow_water_kg_kg` / `graupel_water_kg_kg` (QR/QS/QG — the PRECIPITATING species, **ICON-D2 only**; #530), `cloud_area_fraction_pct` (CLC from ICON-EU, 0–100), `clw_interpolated` (True when CLW filled by spatial interpolation) — populated by GRIB2 enrichment when enabled. The five hydrometeor mixing ratios are enumerated once in `CONDENSATE_LEVEL_FIELDS` (anchor first); the GRIB merge and both interpolation axes derive their field list from it, so a species cannot be carried on one axis and dropped on another.

### HourlyForecast

Single timestep with ~24 optional surface fields (incl. `nwp_k_index` / `nwp_total_totals` — model-native index values from ECMWF GRIB, #294) + optional `nwp_cloud_diagnostics: NWPCloudDiagnostics` (GFS GRIB2 cloud layers) + optional `explicit_convective_diagnostics: NWPExplicitConvectiveDiagnostics` (ICON-D2 explicit-convection storm fields, #462 — sibling of the parameterized diagnostics, never merged into them; its presence is the explicit-convection mode signal) + `pressure_levels: list[PressureLevelData]`. Use `level_at(pressure_hpa)` for quick lookup.
...
| Model | Purpose | Key fields |
|-------|---------|------------|
| `ThermodynamicIndices` | Profile-level indices | LCL/LFC/EL (pressure + altitude), CAPE (surface/MU/ML), CIN, lifted index, showalter, K-index, total totals, precipitable water, freezing/-10C/-20C levels, bulk shear 0-6km/0-1km, sounding_ceiling_ft/nwp_ceiling_ft. **Raw NWP values:** nwp_cape_jkg, nwp_cape_type (sb/ml/mu/unknown), nwp_cin_jkg, nwp_lifted_index, nwp_k_index/nwp_total_totals (#294 — kept separate from the MetPy-derived k_index/total_totals so the two derivations stay independent), nwp_freezing_level_ft, cape_raw_vs_calc_divergent |
| `DerivedLevel` | Per-pressure-level derived values | altitude_ft, temperature_c, dewpoint_c, wet_bulb_c, dewpoint_depression_c, theta_e_k, lapse_rate_c_per_km, relative_humidity_pct, omega_pa_s, w_fpm, richardson_number, bv_freq_squared_per_s2, cloud_liquid_water_g_m3, cloud_liquid_water_g_kg, ice_mixing_ratio_g_kg, rain_water_g_kg (#530), supercooled_rain (#530 — qr ≥ 1e-6 kg/kg below 0 °C; reported, NOT graded, see meteorology-decisions §24), icing_index (Ogimet-DD 0–100), icing_index_nwp (Ogimet-NWP 0–100), sfip_raw, sfip_100, sfip_severity, sfip_variant ("full"/"proxy"/"interp"), clw_interpolated, precip_phase |
| `EnhancedCloudLayer` | Cloud layer from dewpoint depression or NWP diagnostics | base/top (ft + hPa), thickness, mean_temperature_c, coverage (FEW/SCT/BKN/OVC), mean_dewpoint_depression_c, mean_cloud_cover_pct, theoretical_max_top_ft, source ("dd" / "grib" GFS band geometry / "nwp_3d" ECMWF cc + ICON clc / "nwp_condensate" HRRR microphysics, #457 / "synthesized"). The three model-native values are enumerated once in `NATIVE_NWP_CLOUD_SOURCES` — branch on that frozenset, never a hand-written list (two consumers each missed a source doing that, PR #508) |
| `NWPCloudDiagnostics` | NWP-surface diagnostics (GRIB) | low/mid/high (NWPCloudLayerDiag each), convective_cover_pct, convective_base_ft, convective_top_ft, total_cover_pct, boundary_cover_pct, ceiling_ft, freezing_level_ft (ECMWF `deg0l`), ml_cape_jkg/ml_cin_jkg (CIN is NEGATIVE-convention, normalized at decode), convective_precip_mm_h, k_index/total_totals (model-native), lpi_con_max_j_kg + conv_cape_jkg (ICON-EU only, #530 — decoded, no consumer yet). Two non-physical markers: `convective_scheme_absent` (True when the source model has no parameterized convective channel at all — HRRR; routes NWP convective to the CAPE fallback instead of a false NONE) and `band_definition` ("ncep" = LCDC/MCDC/HCDC pressure bands, None for ECMWF/ICON whose bands split much lower and are NOT interchangeable). Both live in `NWP_CLOUD_DIAG_META_FIELDS` so interpolation carries them through unchanged instead of lerping them |
| `NWPExplicitConvectiveDiagnostics` | Explicit-convection storm fields (ICON-D2, #462; `graupel_hour_mm` dropped #468) | source ("icon_d2"), reflectivity_hour_max_dbz, reflectivity_instant_dbz (unfetched v1), echo_top_18dbz_ft (NEVER a cloud top), lightning_potential_hour_max_jkg, updraft_hour_max_ms, updraft_helicity_2_8km_hour_max_m2s2 (signed), detection/strength/echo_top_complete (`strength_complete` = lpi_max + w_ctmax both valid). All values are **corridor extrema** over a ~10 NM route buffer; interval-max fields describe `(H−1, H]`; None ≠ 0 — the completeness flags say whether None means quiet or unavailable |
| `InversionLayer` | Temperature inversion from lapse rate | base/top (ft + hPa), strength_c, surface_based |
| `IcingZone` | Grouped icing zone (Ogimet-DD or Ogimet-NWP) | base/top (ft + hPa), risk, icing_type (RIME/MIXED/CLEAR), sld_risk, supercooled_rain (#530 — any level in the zone; reported, does NOT enter `risk` or `is_hazardous`), mean_temperature_c, mean_wet_bulb_c, mean_rh_pct, mean_icing_index |
...
| `CATRiskLayer` | CAT risk identified by Richardson number | base_ft, top_ft, base/top_pressure_hpa, richardson_number, risk, boundary_layer (#533 — layer wholly inside the boundary layer; still graded, but a SEVERE one does not bypass the route-percentage gate the way free-atmosphere severe CAT does) |
| `VerticalMotionAssessment` | Vertical motion + turbulence | classification, max_omega_pa_s, max_w_fpm, max_w_level_ft, cat_risk_layers, e_shear_layers, convective_contamination, mixed_layer_top_ft (#533 — reported so the CAT suppression below it is inspectable), model_surface_altitude_ft (#541 — the model's own ground in the column's height datum, from its surface pressure; the AGL datum for `boundary_layer` and the cut below which levels are sub-surface extrapolation) |
| `SoundingAnalysis` | Container per model | indices, parcel_path, derived_levels, cloud_layers, nwp_cloud_layers, dd_cloud_layers, icing_zones, icing_ogimet_dd_zones, icing_ogimet_nwp_zones, sfip_zones, ieng_icing_zones, sld_zones, inversion_layers, convective, convective_thermo, convective_nwp, precipitation, vertical_motion, cloud_cover_{low,mid,high}_pct, surface visibility_m/temperature_2m_c/dewpoint_2m_c, nwp_cloud_diagnostics, cloud_method_effective, icing_method_effective, convective_method_effective, convective_nwp_fallback, active_icing_available. **Field semantics:** `cloud_layers`/`icing_zones` are the "active" slots (resolved per user preference by `_resolve_analyses()`). `dd_cloud_layers`/`icing_ogimet_dd_zones` are immutable DD sources, reconstructed by the `_sync_dd_sources` validator on load. `convective` is the active slot; `convective_thermo`/`convective_nwp` retain both derivations (`convective_nwp` carries the `method="nwp_explicit"` assessment on a D2-sourced icon slot, #462; `convective_explicit_unavailable=True` marks an hour whose explicit payload was present but detection-incomplete — `convective_nwp` is then None, deliberately not a quiet NONE). The `*_method_effective` trio records which method `_resolve_analyses` *actually* graded on — the effective, not the requested, method, diverging exactly where a fallback fired (#408): `cloud_method_effective` ("dd"/"nwp"/"nwp_synthesized"), `icing_method_effective` ("ogimet_nwp"/"ogimet_dd"/"sfip_nwp", left None when the method could not run), `convective_method_effective` ("nwp"/"thermo"). The icing/cloud advisory evidence regions source their `method_id` badge from them. `convective_nwp_fallback` (#568, default False) is the marker `convective_method_effective` cannot be: it is True only when the NWP convective track was **requested** and absent, never when thermo was explicitly chosen — the discriminator the §18 DD-amber cap keys on so a model with no native track can reach AMBER but never RED on thermodynamics alone. `convective_nwp is None` cannot substitute (it is False under an explicit thermo request, where the NWP assessment is still computed and stored, just not swapped in). `active_icing_available` (#391, default True) says whether the *active* icing method could run at all here — Ogimet-NWP returns [] on a model with no native cloud envelope, which is otherwise indistinguishable from "ran, found nothing"; the evaluators grade that absence UNAVAILABLE, not clear-by-absence. All downstream consumers read only the active slots. |
...
- `cruise_temperature_c: dict[str, float]` — model → temperature at the elected cruise level; the route graph derives ISA deviation from it
...
Fields: `point_index`, `lat`, `lon`, `distance_from_origin_nm`, `waypoint_icao`, `waypoint_name`, `interpolated_time`, `forecast_hour`, `track_deg`, `wind_components`, `sounding`, `altitude_advisories`, `model_divergence`, `cruise_temperature_c`.
...
| `AdvisoryParameterDef` | Tunable parameter metadata | key, label, description, type (number/percent/altitude/speed/boolean), unit, default, min, max, step, audience ("pilot" inline vs "advanced" behind the expander, #387 — exactly two values, a curation mechanism not a tag taxonomy; defaults to "advanced" so a new param never leaks into the compact view) |
| `AdvisoryCatalogEntry` | Evaluator metadata for UI | id, name, short_description, description, category (icing/cloud/turbulence/convective/model), default_enabled, altitude_dependent, parameters, timing_class ("scan"/"cheap"/"none" — declarative sibling of altitude_dependent; the timing scan asks the registry so a new evaluator auto-participates), timing_hint |
| `ModelAdvisoryResult` | One advisory, one model | status, detail, affected_points, total_points, affected_pct, affected_nm, total_nm, affected_mod_{points,pct,nm} (optional second extent at a higher threshold so the aggregate headlines on real concern rather than the min-threshold union, #300), cross_check (details-only DD-vs-model divergence), mitigations. `build()` classmethod computes pct/nm from counts |
| `RouteAdvisoryResult` | Aggregate across models | advisory_id, aggregate_status, aggregate_detail, per_model list, parameters_used, aggregate_mitigations, representative_model (#393 — first per_model entry matching the aggregate status; the same model that sources detail + mitigations, emitted so clients don't reimplement the rule). `from_per_model()` classmethod aggregates (default MAJORITY, ties broken by worst) |
...
| `AltitudeAdvisoryChange` / `AltitudeAdvisoryDelta` | Diff between a baseline altitude row and a candidate | improved / worsened / unchanged; UNAVAILABLE ignored on either side |
| `Mitigation` + `MitigationKind`/`MitigationSegment`/`MitigationTransition`/`MitigationProfile` | Advice-only decisions that would improve one flagged sub-issue | never alters status; `mitigated_status` is the sub-issue's status if applied, not the advisory's |
| `AdvisoryHighlights` / `HighlightRegion` / `RibbonSegment` / `HighlightSeverity` | Evidence geometry for the cross-section / route ribbon | per-advisory regions carrying the `method_id` badge |
| `Interview` / `InterviewQuestion` / `InterviewOption` | Setup-interview (#387) | an option is a declarative patch over advisory enable flags + params |
...
- **Never assign `HourlyForecast.nwp_cloud_diagnostics` directly** — go through `attach_nwp_diagnostics()`, which mirrors the diagnostics' freezing level onto `freezing_level_m` (the field sounding analysis actually reads). Forgetting the mirror leaves a correct value in the diagnostics and a stale Open-Meteo one in the read path; it was forgotten five times, and `test_no_direct_diagnostics_assignment` now enforces the rule against the source. `nwp_state_snapshot()`/`restore_nwp_state()` roll back both halves together
