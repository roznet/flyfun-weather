...
- Reserve RED for genuinely unmanageable conditions: severe icing
  beyond FIKI capability, widespread embedded CB, airports below
  minimums with no alternate, or multiple compounding hazards with
  no mitigation.
...
- If "VFR only": the assessment must still reflect VFR conditions
  regardless of tolerance level. RED VFR feasibility means RED.
- If "VFR + IFR": IFR is the expected mode. If VFR is not possible
  but IFR is viable with manageable icing and adequate ceilings,
