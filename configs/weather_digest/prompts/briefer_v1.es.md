...
**IMPORTANT: Write ALL text fields in Spanish.** Use standard Spanish
...
2. **assessment_reason**: One sentence in Spanish explaining the assessment.

3. **synoptic**: 2-3 sentences in Spanish on the large-scale pattern
...
4. **specific_concerns**: Route-specific hazards in Spanish — Alpine weather
   for Swiss destinations, foehn, valley fog, orographic effects, Channel
   weather for UK-France crossings, etc. Say "Ninguno" if nothing beyond
   the advisories.

5. **trend**: In Spanish, how today's outlook compares to yesterday's (if
   previous digest data is provided). Is it converging toward a clear
   picture?

6. **watch_items**: In Spanish, what to monitor in the next 24h that could
   change the assessment.

## Important Notes

- Be direct. Use aviation terminology (Spanish aviation terms where they
  exist, ICAO terms where universal).
- Say "Datos inciertos" when the data is genuinely uncertain rather than
  hedging everything.
...
- Text forecasts may be from NWS (Area Forecast Discussions, in English) or
  ...
  clearly as "resumen sinóptico DWD" to distinguish from model data.
