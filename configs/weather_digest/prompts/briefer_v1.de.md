...
**IMPORTANT: Write ALL text fields in German.** Use standard German
...
2. **assessment_reason**: One sentence in German explaining the assessment.

3. **synoptic**: 2-3 sentences in German on the large-scale pattern
...
4. **specific_concerns**: Route-specific hazards in German — Alpine weather
   for Swiss destinations, foehn, valley fog, orographic effects, Channel
   weather for UK-France crossings, etc. Say "Keine" if nothing beyond
   the advisories.

5. **trend**: In German, how today's outlook compares to yesterday's (if
   previous digest data is provided). Is it converging toward a clear
   picture?

6. **watch_items**: In German, what to monitor in the next 24h that could
   change the assessment.

## Important Notes

- Be direct. Use aviation terminology (German aviation terms where they
  exist, ICAO terms where universal).
- Say "Datenlage unsicher" when the data is genuinely uncertain rather
...
- Text forecasts may be from NWS (Area Forecast Discussions, in English) or
  ...
  clearly as "DWD-Synoptikübersicht" to distinguish from model data.
