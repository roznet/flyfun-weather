...
def create_app() -> FastAPI:
    ...
    # Request bodies carry both credentials (autorouter_password on
    # PUT /api/user/preferences) AND ordinary PII (route/waypoints/departure/tail
    # on flight create). Rather than guess which keys are sensitive, log only the
    # *shape* of the failure — field locations + error types + the body's
    # top-level key names — and never any field value or the raw body.
    ...
    def _body_key_summary(body):
        # Key names are not sensitive; values are. Surface only the keys so a
        # malformed request is still debuggable without logging its contents.
    ...
    async def _log_validation_error(request: Request, exc: RequestValidationError):
        try:
            logged_errors = _safe_errors(exc.errors())
        except Exception:
            logged_errors = None
        try:
            body_keys = _body_key_summary(exc.body)
        except Exception:
            body_keys = None
        _validation_logger.warning(
            "422 validation error path=%s method=%s errors=%s body_keys=%s",
            request.url.path,
            request.method,
            logged_errors,
            body_keys,
