...
def generate_skewt(
    ...
    *,
    ...
    # Wind barbs per level, NaN where absent (mirrors prepare.py). A partial
    # GRIB fetch can leave some levels without u/v; an all-or-nothing gate would
    # drop barbs for the whole column, including the levels that do have wind.
    # NaN levels simply render no barb (matplotlib skips them), so the levels
    # that arrived stay barbed (#478).
    has_any_wind = any(
...
def generate_hodograph(
    ...
    # Per-level wind: a partial GRIB fetch can leave some levels without u/v,
    # so build the hodograph from the levels that DO carry wind instead of
    # failing the whole plot all-or-nothing. Still refuse below 3 points — a
    # hodograph from one or two levels would be misleading, not merely sparse
    # (#478).
    wind_levels = [
        pl for pl in levels
