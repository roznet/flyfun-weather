...
**Prior updates:** 2026-03-23, 2026-04-20 (fresh first-principles review), 2026-05-20 (magic-link / rolling-session auth surface), 2026-06-11 (five parallel blind workstreams), 2026-06-20 (OAuth 2.1 / MCP-OAuth / DCR / GDPR full-stack pass), 2026-06-28 (iOS-client-focused pass)
**Latest update / Current audit:** 2026-08-08 (seventh full-stack pass — six parallel independent workstreams run **blind** to this document — auth/sessions/OAuth, API authorization/IDOR, injection/SSRF/prompt-injection, frontend/XSS/CSP, secrets/deploy/dependencies, iOS + `flyfun-common` Swift — then reconciled against it. `flyfun-common` Python **and** Swift both in-checkout (`/home/user/flyfun-common`, prod pin `flyfun-common>=0.6.2`). Targeted the surface that postdates 2026-06-28: verification data-tiering (#522), `briefer_v3`, digest-eval-against-external-repo, and the sliding-session / OAuth-token-lifecycle code. See the 2026-08-08 section immediately below.)
...
Seventh full-stack pass. Six workstreams (auth/sessions/OAuth · API authz/IDOR · injection/SSRF/prompt-injection · frontend/XSS/CSP · secrets/deploy/deps · iOS + Swift auth) each ran **blind** — instructed not to read this document — then were reconciled against it. Every new finding below was re-verified against source by the reconciler (line numbers current as of this pass); the two headline items (2026-08-H1, 2026-08-M1) and the DCR-IP regression were read line-by-line before being written up. Both `flyfun-common` Python and Swift trees were in-checkout, so OAuth/session and iOS findings rest on real library code. New surface since the 2026-06-28 pass — verification data-tiering (#522), `briefer_v3`, the digest eval running against an external labelled repo, and the sliding-session / OAuth-token-lifecycle code — was specifically covered.

### Summary

The durable picture holds and, in several places, has **improved** since June: no SQLi, no command injection, no path traversal, no SSRF, no unsafe deserialization / XXE / SSTI / ReDoS (all re-verified from scratch); object-level authorization on **writes** is uniformly owner-gated; admin gating is complete; there is no mass-assignment surface; the frontend escapes user data at the high-value sinks (the 2026-06-H1 profile-name stored-XSS stays fixed); PKCE `S256`, JWT algorithm-pinning, OAuth consent-fingerprint CSRF binding, redirect-URI exact-match, and magic-link enumeration-safety are all sound. Three surfaces the last passes flagged are now materially better: the **iOS OAuth deep-link (long-open H8) has been implemented** as a `code`+`state` HTTPS exchange (the `oauth-deeplink-hardening` design shipped), the **digest/briefer prompt no longer ingests any attacker-controlled free text** (closing the 06-11 M2 residual), and the MySQL grant is now **DML-only least-privilege**.

The new risk this pass concentrates in the **session-revocation and token-lifecycle** code, plus a **read-side data-exposure** the write-side rigor doesn't cover:

- **One confirmed High:** the sliding-session middleware re-mints JWTs with **no revocation-epoch check**, so "log out everywhere" (the sole server-side kill-switch for a stolen session) can be permanently defeated by hitting any public 200 endpoint. Combined with the fact that logout-all also does not revoke `ff_` API tokens, OAuth access tokens, or refresh tokens, there is currently **no effective way to revoke a compromised credential short of rotating `JWT_SECRET`**.
- **A read-side cross-tenant exposure:** flights are **public by default** and their IDs carry only **16 bits** of guessable entropy, so any authenticated user can enumerate a target pilot's briefing (route, schedule, altitudes, per-model weather, AI digest, PDF) for a known route/date with ≤65k GETs — no share link required.
- **A regression in the 06-20 H1 fix:** the new DCR per-IP rate limiter reads `request.client.host`, which behind Caddy is the proxy IP, collapsing the per-attacker throttle onto one global bucket.

### New findings (this pass)

| ID | Sev | Headline |
|----|-----|----------|
| 2026-08-H1 | **High** | **Sliding-session middleware bypasses session-epoch revocation.** `SlidingSessionMiddleware._maybe_refresh_token` (`flyfun-common .../auth/middleware.py:133-165`) decodes signature+`exp` only, does **no DB access**, and mints a successor token with a fresh `iat` — it never consults `users.tokens_valid_after`. The `status < 400` guard (`:96-99`) only skips reissue when the response itself rejects; any public endpoint that returns 200 while the credential is present (`GET /health`, `/whats-new`, `/auth/providers`, static/index) triggers a refresh. A stolen cookie/bearer within its 15-day refresh window survives the victim's `logout-all`: the thief requests `/health`, receives `Set-Cookie`/`X-Renewed-Token` with `iat > tokens_valid_after`, and the epoch check (`db/deps.py:185-193`) now passes everywhere — indefinitely, since each near-expiry refresh renews `iat`. The one server-side kill-switch for a compromised session is defeated. |
| 2026-08-M1 | Medium | **Public-by-default flights + 16-bit guessable IDs → cross-tenant briefing enumeration.** `private` defaults `False` (`db/models.py:110`, `CreateFlightRequest.private=False` at `flights.py:199`) and `_load_flight_or_404` (`flights.py:2122`) rejects only *private* non-owned flights, so any authenticated caller can read another user's non-private flight through the dozens of viewer-gated read endpoints (`GET /{flight_id}`, `/export`, all of `packs.py` list/get/report.html/report.pdf/charts, the `agent.py` MCP reads). The ID is `{route_name}-{YYYY-MM-DD}-{4 hex}` (`flights.py:260-273`) — `route_name` (waypoint list) and date are guessable for a target, leaving only **65,536** values in the truncated SHA-256. ≤65k GETs enumerate any pilot's flight on a chosen route/date and return route, departure/alternate times, altitudes, per-model assessments, AI digest, and rendered HTML/PDF. Tail number/nickname are correctly owner-gated (`flights.py:365-371`), so the leak is route/schedule/weather, not registration. This is the *amplifying combination* the sharing feature doesn't require: opt-out privacy **and** a guessable ID mean exposure doesn't depend on the owner ever sharing a `/s/{code}` link. |
| 2026-08-M2 | Medium | **DCR per-IP rate limiter is blind behind the proxy (regression in the 06-20 H1 fix).** `POST /oauth/register` derives the client IP as `request.client.host if request.client else None` (`flyfun-common .../oauth/router.py:256`) — behind Caddy this is the constant proxy address, so the per-IP sliding window (`register_rate_per_hour=5`) collapses onto a single world-shared bucket and `registered_ip` is logged as the proxy IP. Every other limiter in the codebase correctly uses the `X-Real-IP` / rightmost-XFF helper (`magic_link._client_ip`, `security._client_ip`); this one does not. Only the global 50/hr backstop remains, so per-attacker throttling of consent-phishing-client / DB-spam registration is effectively absent. |
| 2026-08-M3 | Medium | **`logout-all` / `tokens_valid_after` revokes only JWT sessions — not `ff_` API tokens, OAuth access tokens, or refresh tokens.** `_is_session_revoked` returns `False` whenever `token_iat is None` (`db/deps.py:185-193`), and `ff_`/OAuth bearer tokens authenticate via `_authenticate_bearer_token` with `token_iat=None` (`:179-180`); refresh tokens are never consulted. So after a compromise-triggered "log out everywhere," every leaked connector access token (≤7 d) and refresh token (sliding 90 d) stays live and keeps minting. This is the deferred H3-2026-06-20 revoke-all residual, now **confirmed** and materially compounding 2026-08-H1: together they mean the only real kill-switch is a global `JWT_SECRET` rotation. |
| 2026-08-M4 | Medium | **OAuth auth-code redemption and refresh rotation are non-atomic, and rotation lacks reuse-family revocation.** `POST /oauth/token` reads `auth_code.used` then later sets it (`oauth/router.py:569-660`) with no `SELECT … FOR UPDATE` / conditional `UPDATE … WHERE used=0`; on MySQL InnoDB REPEATABLE-READ two concurrent redemptions of one code can both issue token pairs (the RFC 6749 §10.5 replay-revocation only fires on *sequential* reuse). Refresh rotation (`:686-767`) has the same race and, on presentation of an already-rotated (revoked) refresh token, returns `invalid_grant` **without** revoking the descendant family — so a stolen-then-rotated refresh chain survives undetected (no OAuth 2.1 breach-detection). Carry-forward M-new-7, re-confirmed with the added family-revocation gap. |
| 2026-08-L1 | Low | **One-click admin-approve is a state-changing GET with no interstitial.** `GET /admin/approve/{user_id}` (`weatherbrief/api/admin.py:581-632`) approves purely from the HMAC-signed link. The magic-link subsystem deliberately uses a non-consuming pass-through page *because* corporate mail scanners (Proofpoint/Mimecast/ATP) pre-click links; the approval link has the same exposure — an admin's scanner pre-fetching it auto-approves the account with no admin intent. Add a POST confirmation interstitial (mirrors the magic-link pattern). Sharpens M-new-3. |
| 2026-08-L2 | Low | **Frontend escaping inconsistencies (drift-prone innerHTML sinks).** Unescaped server `err`/`detail` into `innerHTML` at `web/ts/admin-main.ts:160` and `:414` and `web/ts/visualization/climatology-tab.ts:329` (the same files escape correctly elsewhere — `admin-main.ts:649` uses `escapeHtml`); partial attribute escaping in `web/ts/helpers/metrics-helper.ts:252` (hand-rolled `.replace(/"/g,…)` misses `<>'`); unescaped server airport names in shareable-briefing sinks (`web/ts/managers/briefing-ui.ts:2816,2979`); `web/settings.html:225,257` `target="_blank"` without `rel="noopener"`. None attacker-controlled today (enum/DB-sourced), but fragile, and CSP `'unsafe-inline'` (M-new-1) would not backstop them. Refresh of M-new-19 / L7 with current lines. |
| 2026-08-L3 | Low | **iOS: public-data caches lack a file-protection class; keychain-accessibility guarantee is untested.** `AirportDatabase.swift:143` and `HelpCatalogStore.swift:79` write with `.atomic` only (no `.completeFileProtection…`) — content is public (AIRAC airport list, help text), so not an exposure, but inconsistent with the (correctly protected) briefing/PIREP stores. Separately, the JWT keychain item's `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` guarantee is applied at the call site (`KeychainBearerTokenStore.swift:16-21`) but enforced inside the external `rzutils` `CodableSecureStorage` — add an integration test asserting the stored item's accessibility so a future `rzutils` bump can't silently downgrade it. |
| 2026-08-L4 | Low | **Dependency-confusion window on `flyfun-common`.** `pyproject.toml` requires `flyfun-common>=0.6.2` and the Dockerfile installs from the default PyPI index with no `--index-url` / no lockfile hashes (`Dockerfile:32`). If that name is not actually claimed on public PyPI, an attacker can register it and win resolution on the next build. Confirm the name is reserved on PyPI (or install it explicitly from the git URL / private index). Folds into H6. |
| 2026-08-I1 | Info | **Autorouter OAuth error paths still log full response body / `token_data`.** `flyfun-common .../autorouter.py:152-156,164` log `resp.text` and the whole `token_data` dict on the token-exchange error / missing-`access_token` branch — a provider error body could carry a partial token / `refresh_token` / echoed `code`. Log status + field names only. Carry-forward M-new-14 / L6, still open. |

### Detail on 2026-08-H1 (High) — session-epoch revocation is bypassable

The rolling-session model (30-day JWT, re-minted once remaining lifetime drops below 15 days) is enforced by `SlidingSessionMiddleware`. `logout-all` (`auth/router.py`) and compromise-recovery set `users.tokens_valid_after`; `_is_session_revoked` (`db/deps.py:185-193`) then rejects any JWT whose `iat` predates that epoch — **but only inside the DB-backed dependencies** `current_user_id` / `optional_user_id`. The middleware runs on every request independent of the route's auth dependency and, in `_maybe_refresh_token`, does:

```python
payload = decode_token(current_token, get_jwt_secret())   # signature + exp only, no DB
...
return create_token(sub, payload.get("email",""), payload.get("name",""), get_jwt_secret())  # iat = now
```

There is no `tokens_valid_after` lookup (no DB session in the middleware at all), so the reissued token's `iat = now` is necessarily **after** the epoch. The `status < 400` guard prevents reissue only when the response itself rejects; a public 200 endpoint carrying the still-signature-valid cookie/bearer (e.g. `GET /health` at `weatherbrief/api/app.py`) reissues normally. An attacker with a stolen credential in its refresh window hits `/health`, harvests the fresh `Set-Cookie` / `X-Renewed-Token`, and now holds a token with `iat > tokens_valid_after` that passes the epoch check on every protected route — and each subsequent near-expiry refresh renews `iat`, so the access is indefinite and the victim's "log out everywhere" is permanently defeated.

**Remediation:** run the epoch/`approved` check *before* reissue — load the user in the middleware and refuse to refresh when `iat < tokens_valid_after` or the user is suspended; or carry the original `iat` / an `auth_time` claim forward unchanged so a rolled token can never cross the epoch; or move the refresh behind the authenticated dependency instead of blanket middleware. Fix **2026-08-M3** in the same change (revoke `ff_`/OAuth/refresh tokens on `logout-all`) so a single control actually kills a compromised credential.

### Reconciliation — carry-forward statuses re-verified 2026-08-08

| Prior ID | Title | Status | Evidence (this pass) |
|----------|-------|--------|----------------------|
| H8 (iOS deep-link) | JWT in custom-scheme `?token=` + loose regex | **FIXED / implemented** | iOS now uses `FlyFunAuthService` `code`+`state` HTTPS exchange (CSPRNG `state` verified on callback; token never in a URL). The bare-token deep link survives only as a signed-out + `scope:"review"`-gated reviewer path (`AppState.swift:258-264`). The `oauth-deeplink-hardening` design shipped. |
| M2 (06-11) digest prompt-injection | METAR/SIGMET/waypoint fencing residual | **Effectively CLOSED** | `digest/prompt_builder.py` composes only quantitative data + government METAR/TAF/SIGMET + trusted-nav-DB labels; user `route_name` does **not** enter the prompt; `llm_digest.py` binds no tools and returns a `Literal` enum. No attacker-controlled free text reaches the digest LLM. |
| H2 | Docker net segmentation + broad MySQL grant | **OPEN (improved)** | Flat `shared-services` net and `'weatherbrief'@'172.%'` grant unchanged, but the grant is now **DML-only** (no `CREATE/ALTER/DROP/INDEX`) — a compromised app credential can't alter schema. Narrow the `172.%` host mask. |
| H3 / L4 | Single `JWT_SECRET` reuse | **OPEN (re-verified)** | Still signs JWTs + Starlette session + native exchange codes + pack-HMAC; dev Fernet key derives from it (`config.py:86-96`, `encryption.py:24-26`, `storage/flights.py:175`). Session JWT still carries no `aud`/`iss`. |
| H5 | Pack-HMAC mismatch served; NULL trusted | **OPEN (re-verified)** | `_verify_pack_hmac` returns `True` for NULL hmac (`storage/flights.py:209-210`); callers `logger.warning` then serve the row on mismatch (`:712-717`, `:736-740`). Unchanged. |
| H6 | No Python lockfile / unpinned deps | **OPEN (re-verified)** | All `>=` floors incl. `flyfun-common>=0.6.2`; `Dockerfile:32` `pip install -e .`; no `requirements/constraints/uv.lock`; base images on moving tags (`python:3.13-slim`, `node:22-slim`); no Dependabot/`pip-audit`. New angle: 2026-08-L4 dependency-confusion. npm side still correct (`npm ci --ignore-scripts` + committed lockfile). |
| H7 | Rate-limit auth/token endpoints | **PARTIAL (unchanged)** | DCR limited (06-20) but per-IP-blind (2026-08-M2); `api/tokens.py` create + `/admin/approve` remain unthrottled. |
| M-new-9 | `/api/refresh/active` cross-tenant leak | **OPEN (re-verified)** | `get_active_refreshes` (`packs.py:2587-2593`) returns `get_all_active()` for every user, unscoped, to any authenticated caller. |
| M-prior-8 | `_load_flight_or_404` fail-open default | **OPEN (re-verified)** | `flights.py:2122` still `viewer_id: str \| None = None`; `_get_pack_dir` (`packs.py:4513`) shares the default. Every current caller passes `viewer_id=`, so latent — but 2026-08-M1 shows the *public-flight* branch is already a live read exposure. |
| M-new-1 | CSP `script-src 'unsafe-inline'` | **OPEN (re-verified)** | `deploy/weather.flyfun.aero.caddy:10` still allows `'unsafe-inline'` for script+style (forced by per-page inline theme-setter scripts + one inline `onclick`); `object-src 'none'` also absent. Would not backstop a DOM-XSS. |
| M2 (06-20) scope model | Unregistered scope ⇒ allow | **PARTIAL** | Registered *limited* scopes are default-deny and `optional_user_id` raises 403 on insufficient scope, but the "unregistered ⇒ full" footgun for `mcp` is unchanged. Not exploitable today (API re-enforces per request). |
| M-new-3 | Admin approval link no one-time-use | **OPEN (+ new angle 2026-08-L1)** | HMAC + TTL only; now also flagged as a scanner-clickable state-changing GET. |
| M-new-14 / L6 | Autorouter error-body logging | **OPEN (re-verified)** → 2026-08-I1 | `autorouter.py:152-156,164`. |
| C1 / triage PI | Feedback-triage prompt injection | **FIXED (intact)** | `_assert_sandboxed()` UID gate before any LLM call, per-invocation random-delimiter fencing (`triage/security.py`, `triage/prompt.py`), `--tools Read,Grep,Glob` (no Agent), `scan_for_exfil` output gate, PII withheld — all re-verified. |
| 2026-06-H1 | Profile-name stored XSS in digest banner | **FIXED (intact)** | `escapeHtml` applied at the sink; verified alongside ~consistent escaping across the high-value frontend sinks. |
| OAuth 06-20 H1/H2/H3 | DCR throttle / consent fingerprint / refresh expiry | **Present (deployed via pin)** | Consent POST bound to `sha256(client_id\|redirect_uri\|scope\|code_challenge)` (`oauth/router.py:83-96,430-459`); refresh tokens carry a 90-day sliding expiry; DCR is rate-limited — **but** see 2026-08-M2 (the DCR limiter is proxy-IP-blind) and 2026-08-M3/M4 (no revoke-all; non-atomic redemption; no refresh-family revocation). |

### Positives re-confirmed from scratch this pass

ORM-only DB access (the sole f-string SQL is internal schema/column names in `tasks/snapshot_artifact.py`, never request data; `nav.py` `ATTACH DATABASE ?` is parameterized against a server-config path); `safe_path_component` + ICAO/model/chart-id/run-cycle allowlists on every file-serving path (traversal bypass attempted, none found); no `eval`/`exec`/`shell=True`/`pickle`/unsafe-`yaml`/XML-parser anywhere; no user-controlled outbound URL (no SSRF); the two `subprocess` sites use fixed argv (no shell); Jinja is `autoescape=True` on package templates only (no SSTI). Writes uniformly owner-gated (`_load_owned_flight`, `_validate_{aircraft,profile,pack}_ownership`); owner-only side effects (scan scheduling, recalc, digest generate) separated from viewer reads; no mass-assignment (`is_admin`/`approved`/`credits`/`permissions` never client-settable); admin gating complete across every admin router; Stripe webhook signature-verified and receipt recipient derived from the Stripe session. JWT decode pinned `HS256` / Apple `RS256`+aud+iss; PKCE `S256` mandatory-when-present; OAuth consent fingerprint-bound; redirect URIs exact-matched; account deletion cleans OAuth grants + anonymizes PIREPs; magic-link enumeration-safe with per-token OTP attempt cap. iOS: device-bound Keychain (`…ThisDeviceOnly`), no ATS exceptions / no `http://` / no `WKWebView`, briefing/PIREP caches file-protected + (cache) backup-excluded + per-user scoped, JWT never logged, all dev/UI-test auth bypasses `#if DEBUG`-gated. Deploy: non-root UID 2000, ports bound `127.0.0.1`, `.env` git+docker-ignored, no secrets in tracked files, strong Caddy header suite (HSTS preload, nosniff, frame-ancestors, Referrer-Policy, Permissions-Policy, `-Server`), `/ts/*`+`*.map` 404'd at the edge, `/docs` disabled in prod, prod CORS regex-scoped to `claude.ai`/`claude.com`, dedicated secret-exfil scanner + 422-handler that logs key-names-only.

### Priority roadmap (this pass)

1. **2026-08-H1 + 2026-08-M3** — make session/token revocation actually work: epoch-check (and `approved`-check) in `SlidingSessionMiddleware` *before* reissue, and have `logout-all` revoke `ff_`/OAuth access + refresh tokens. Together these restore a real "log out everywhere" / compromise kill-switch and remove the current dependence on rotating `JWT_SECRET`.
2. **2026-08-M1** — default new flights `private=True` (share-link / subscription grants read) and/or widen `_compute_flight_id` to a full-length unguessable token so possession of the link is the capability.
3. **2026-08-M2** — route DCR through the trusted-IP helper (`X-Real-IP` / rightmost-XFF); consider Caddy `trusted_proxies` + `header_up X-Forwarded-For {remote_host}`.
4. **2026-08-M4** — atomic single-use on auth-code redemption + refresh rotation (`UPDATE … WHERE used=0`, check rowcount); revoke the refresh-token family on rotated-token reuse.
5. **Carry-forwards, highest-leverage:** H5 (reject on pack-HMAC mismatch + `NOT NULL` backfill), H6 (lockfile + SHA-pin `flyfun-common`/base images + confirm PyPI name — 2026-08-L4, + Dependabot/`pip-audit`), H7 (rate-limit `api/tokens` + `/admin/approve`), H2 (network split + narrow MySQL host mask), H3/L4 (split the master secret via HKDF), M-new-9 (`/refresh/active` scope to caller/admin), M-prior-8 (require `viewer_id`), M-new-1 (drop `'unsafe-inline'` via nonces; add `object-src 'none'`).
6. **Lows / hardening:** 2026-08-L1 (approve interstitial), 2026-08-L2 (escape the drift-prone innerHTML sinks + `rel="noopener"`), 2026-08-L3 (iOS cache protection + keychain-accessibility test), 2026-08-I1 (Autorouter error logging), M-new-2 (container `cap_drop`/`no-new-privileges`/`read_only`), M-new-17 (`author_association` gate on the `@claude` workflow + SHA-pin actions).
